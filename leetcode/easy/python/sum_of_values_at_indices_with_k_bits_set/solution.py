    ...
    def sum_indices_with_k_set_bits(numbers: List[int], bit_count: int) -> int:
        ...
        return sum(
            ...
            for i, number in enumerate(
                islice(iter(numbers), 2**bit_count - 1, None)
            )
            if (i + 2**bit_count - 1).bit_count() == bit_count
        )
...
class TestSumIndicesWithKSetBits(TestCase):
    ...
    def test_minimum_bit_count_with_nonzero_sum(self):
        expected = 1
        result = Solution.sum_indices_with_k_set_bits([1], 0)
        self.assertEqual(expected, result)
